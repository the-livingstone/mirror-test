...
class OrdersRepo:
    ...
    def get_day_orders(self, day: date) -> list[Order]:
        items = self.session.query(OrderORM).where(OrderORM.start_day == day.isoformat()).order_by(OrderORM.start_time).all()
    ...
    def get_free_walkers(self, start_time: datetime) -> list[Walker]:
        ...
        free_walkers = self.session.query(WalkerORM).where(WalkerORM.uid.not_in(orders_at_time))
